import { businessConfigInfo, domain } from "@/data/constants";
...
export const whatsappMsgs = (
  ...
  if (name === "justContact") {
    ...
    return `https://wa.me/${businessConfigInfo.whatsapp}?text=Nueva%20recarga%20de%20Saldo%20id%3A%0A%0A${balanceRechargeInfo?.balanceRechargeId}%2C%20recarga%20realizada%20por%3A%20%0A%0A${username}%2C%20monto%3A%20%0A%0A${balanceRechargeInfo?.balanceType.price}%20Por%20medio%20de%3A%0A%0A${balanceRechargeInfo?.payment_method.payment_method.name}%20usando%3A%0A%0A${balanceRechargeInfo?.payment_method.payment_method_description}.`;
  }
};
